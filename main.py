...
def tkinter_app():
    ...
    tk.Button(covidTab, text= "Daily and %Change in daily Cases", command=lambda: dailyAndCumulativeCasesForm(form)).place(x=100, y=40)
    tk.Button(covidTab, text= "Compare two Regions", command=lambda: compareTwoRegionsForm(form)).place(x=340, y=40) 
    tk.Button(covidTab, text= "Top regions with the highest cases", command=lambda: topFiveRegionWithHighestCasesForm(form)).place(x=100, y=70)
    tk.Button(covidTab, text= "View cases by Months", command=lambda: viewByMonthsForm(form)).place(x=350, y=70)  
